...
func execute_start(graphs: Array[Dictionary], runtime_state, gameplay_context: Dictionary = {}) -> Dictionary:
    if runtime_state == null or not runtime_state.is_loaded():
        return _failure("Visual graph Play runtime requires loaded disposable world state.")
    var compiled: Dictionary = Compiler.new().compile_registry(graphs)
    if not compiled.get("ok", false):
        return compiled
    var plans: Dictionary = compiled["plans"]
    var graph_ids: Array[String] = []
    for graph_id in plans.keys():
    ...
    for graph_id in graph_ids:
        ...
        if str(plan.get("kind", "")) != "event" or not bool(plan.get("enabled", true)):
            continue
        var interpreter = Interpreter.new()
        interpreter.step_budget = step_budget_per_graph
        var context: Dictionary = gameplay_context.duplicate(false)
        context["plans"] = plans
        context["get_entity_position"] = Callable(self, "_get_entity_position").bind(runtime_state)
        context["set_entity_position"] = Callable(self, "_set_entity_position").bind(runtime_state)
        var result: Dictionary = interpreter.execute(plan, context)
        if not result.get("ok", false):
            return {
                "ok": false,
                "errors": ["Visual graph %s failed during Play: %s" % [graph_id, str(result.get("errors", []))]],
                "graph_id": graph_id,
            }
        total_steps += int(result.get("steps", 0))
        executed += 1
        for value in result.get("trace", []):
...
func _set_entity_position(entity_id: String, value: Variant, runtime_state) -> Dictionary:
    ...
    if value is Vector3:
        ...
        position = Vector3(float(value[0]), float(value[1]), float(value[2]))
    else:
        return _failure("Set Position requires a Vector3-compatible value.")
