...
func set_mode_enabled(mode: StringName, enabled: bool) -> Dictionary:
    match mode:
        ...
        _:
            return {"ok": false, "errors": ["Unknown snapping mode: %s" % mode]}
...
func snap_position(value: Vector3) -> Vector3:
    if not grid_enabled or grid_size <= 0.0:
        return value
    return Vector3(
        snappedf(value.x, grid_size),
        snappedf(value.y, grid_size),
        snappedf(value.z, grid_size)
    )
...
func snap_rotation(value: Vector3) -> Vector3:
    if angle_step_degrees <= 0.0:
        return value
    return Vector3(
        snappedf(value.x, angle_step_degrees),
        snappedf(value.y, angle_step_degrees),
        snappedf(value.z, angle_step_degrees)
    )


func drop_to_ground(value: Vector3, ground_y: float = 0.0) -> Vector3:
    return Vector3(value.x, ground_y, value.z)
...
func snap_to_object(value: Vector3, candidates: Array) -> Dictionary:
    return _nearest_candidate(value, candidates, object_threshold, "object")


func snap_to_socket(value: Vector3, candidates: Array) -> Dictionary:
    return _nearest_candidate(value, candidates, socket_threshold, "socket")
...
func resolve_position(value: Vector3, context: Dictionary = {}) -> Dictionary:
    ...
    if surface_enabled and context.has("surface_position"):
        ...
        result = snap_to_surface(surface_position, surface_normal, float(context.get("surface_offset", 0.5)))
        mode = "surface"

    if object_enabled:
        var object_result := snap_to_object(result, context.get("object_candidates", []))
        if object_result.get("snapped", false):
            result = object_result["position"]
            mode = "object"

    if socket_enabled:
        var socket_result := snap_to_socket(result, context.get("socket_candidates", []))
        if socket_result.get("snapped", false):
            result = socket_result["position"]
            mode = "socket"
...
func _nearest_candidate(value: Vector3, candidates: Array, threshold: float, kind: String) -> Dictionary:
    var best_id := ""
    var best_position := value
    var best_distance := INF
    for item in candidates:
        if not item is Dictionary or not item.has("position"):
            continue
