...
func _init() -> void:
    name = "EditorSession"
    _bridge.name = "RuntimeEntityBridge"; add_child(_bridge); add_child(_ghost)
    _selection.bind_bridge(_bridge); _selection.selection_changed.connect(_on_selection_changed)
...
func bind_project(project, dirty_callback: Callable) -> Dictionary:
    ...
    _project = project; _dirty_callback = dirty_callback; _cell_resolver = Callable(); _ground_resolver = Callable(); _socket_resolver = Callable()
    var result: Dictionary = _bridge.rebuild(_project.entity_records)
    if not result.get("ok", false): _project = null; _dirty_callback = Callable(); return result
...
func bind_cell_resolver(resolver: Callable) -> Dictionary:
    ...
    _cell_resolver = resolver; return {"ok": true, "errors": []}
func bind_ground_resolver(resolver: Callable) -> Dictionary:
    ...
    _ground_resolver = resolver; return {"ok": true, "errors": []}
func bind_socket_resolver(resolver: Callable) -> Dictionary:
    ...
    _socket_resolver = resolver; return {"ok": true, "errors": []}
...
func refresh_runtime(preserve_selection: bool = true) -> Dictionary:
    ...
    var selected: Array[String] = []; var primary := ""
    if preserve_selection: selected = _selection.get_selected_ids(); primary = _selection.get_primary_entity_id()
    _selection.clear()
    var result: Dictionary = _bridge.rebuild(_project.entity_records)
...
func begin_proxy_placement(display_name: String = "Proxy Object") -> Dictionary:
    ...
    _ghost.show_record(record); _sync_ghost_cell(); placement_changed.emit(true, _ghost.get_record())
...
func update_placement_preview(position_value: Vector3, context: Dictionary = {}) -> Dictionary:
    ...
    var record: Dictionary = _ghost.get_record(); var transform_data: Dictionary = record.get("transform", {}).duplicate(true)
    var rotation_value: Vector3 = _vector3(transform_data.get("rotation_degrees", [0.0, 0.0, 0.0])); var scale_value: Vector3 = _vector3(transform_data.get("scale", [1.0, 1.0, 1.0]))
    if str(snap_result.get("mode", "")) == "surface" and resolved_context.has("surface_normal"): rotation_value = _snapping.surface_rotation(resolved_context.get("surface_normal", Vector3.UP))
...
func commit_placement() -> Dictionary:
    if not _ghost.is_active(): return _failure("No placement preview is active.")
    var command = PlaceEntityCommand.new(_project, _ghost.get_record()); var result: Dictionary = _history.execute_command(command, "Place object")
    if not result.get("ok", false): return _history_failure(result)
    var entity_id: String = command.get_entity_id(); _ghost.hide_preview(); placement_changed.emit(false, {})
    var finish: Dictionary = _finish_mutation(false)
    if not finish.get("ok", false): return finish
    _selection.select_single(entity_id); finish["entity_id"] = entity_id; return finish
...
func nudge_selected(mode: StringName, delta: Vector3) -> Dictionary:
    ...
    var constrained: Vector3 = _gizmo.constrain(delta); var updates: Dictionary = {}
    for entity_id in _selection.get_selected_ids():
        ...
        var position_value: Vector3 = _vector3(transform_data.get("position", [0.0, 0.0, 0.0])); var rotation_value: Vector3 = _vector3(transform_data.get("rotation_degrees", [0.0, 0.0, 0.0])); var scale_value: Vector3 = _vector3(transform_data.get("scale", [1.0, 1.0, 1.0]))
        match mode:
...
func move_selection_to(target: Vector3) -> Dictionary:
    ...
    var record: Dictionary = _find_record(primary_id); var current: Vector3 = _vector3(record.get("transform", {}).get("position", [0.0, 0.0, 0.0]))
...
func drop_selection_to_ground(ground_y: float = 0.0) -> Dictionary:
    ...
    var minimum_y: float = _runtime_min_y(primary_node, current.y - 0.5)
...
func snap_selection_to_surface(hit_position: Vector3, hit_normal: Vector3) -> Dictionary:
    ...
    var primary_id: String = _selection.get_primary_entity_id(); var primary: Dictionary = _find_record(primary_id)
    ...
    var target: Vector3 = _snapping.snap_to_surface(hit_position, hit_normal); var delta: Vector3 = target - primary_position; var rotation: Vector3 = _snapping.surface_rotation(hit_normal); var updates: Dictionary = {}
    for entity_id in _selection.get_selected_ids():
        var record: Dictionary = _find_record(entity_id); var transform_data: Dictionary = record.get("transform", {}).duplicate(true)
        var position_value: Vector3 = _vector3(transform_data.get("position", [0.0, 0.0, 0.0])) + delta; var scale_value: Vector3 = _vector3(transform_data.get("scale", [1.0, 1.0, 1.0])); var cell_id: String = str(record.get("cell_id", ""))
        var resolved_cell: String = _resolve_cell_id(position_value); if not resolved_cell.is_empty(): cell_id = resolved_cell
...
func duplicate_selected() -> Dictionary:
    var ids: Array[String] = _selection.get_selected_ids(); if ids.is_empty(): return _failure("Duplicate requires a selection.")
    var command = DuplicateEntitiesCommand.new(_project, ids, Vector3(_snapping.grid_size, 0.0, _snapping.grid_size)); var result: Dictionary = _history.execute_command(command, "Duplicate selection")
    if not result.get("ok", false): return _history_failure(result)
    var finish: Dictionary = _finish_mutation(false); if finish.get("ok", false): _selection.set_selected(command.created_ids()); finish["entity_ids"] = command.created_ids()
    return finish
func delete_selected() -> Dictionary:
    var ids: Array[String] = _selection.get_selected_ids(); if ids.is_empty(): return _failure("Delete requires a selection.")
    var result: Dictionary = _history.execute_command(DeleteEntitiesCommand.new(_project, ids), "Delete selection"); if not result.get("ok", false): return _history_failure(result)
    _selection.clear(); return _finish_mutation(false)
func group_selected() -> Dictionary:
    var ids: Array[String] = _selection.get_selected_ids(); if ids.size() < 2: return _failure("Grouping requires at least two selected entities.")
    var command = GroupEntitiesCommand.new(_project, ids); var result: Dictionary = _history.execute_command(command, "Group selection"); if not result.get("ok", false): return _history_failure(result)
    var finish: Dictionary = _finish_mutation(false); if finish.get("ok", false): _selection.select_single(command.group_id()); finish["group_id"] = command.group_id()
    return finish
func undo_edit() -> Dictionary:
    var result: Dictionary = _history.undo(); if not result.get("ok", false): return _history_failure(result)
    _selection.clear(); return _finish_mutation(false)
func redo_edit() -> Dictionary:
    var result: Dictionary = _history.redo(); if not result.get("ok", false): return _history_failure(result)
    _selection.clear(); return _finish_mutation(false)
...
func _finish_mutation(preserve_selection: bool) -> Dictionary:
    var refresh: Dictionary = refresh_runtime(preserve_selection); if not refresh.get("ok", false): return refresh
    ...
    var data: Dictionary = _project.to_dictionary(); project_changed.emit(data); return {"ok": true, "errors": [], "project_data": data}

func _snap_selection_to_candidates(candidates: Array, kind: StringName) -> Dictionary:
    var primary_id: String = _selection.get_primary_entity_id(); if primary_id.is_empty(): return _failure("Snapping requires a primary selection.")
    var record: Dictionary = _find_record(primary_id); var current: Vector3 = _vector3(record.get("transform", {}).get("position", [0.0, 0.0, 0.0])); var result: Dictionary
    match kind:
    ...
    var move_result: Dictionary = move_selection_to(result["position"]); if move_result.get("ok", false): move_result["snap_id"] = result["id"]
    return move_result

func _runtime_snap_candidates(socket: bool) -> Array[Dictionary]:
    var result: Array[Dictionary] = []
    for entity_id in _bridge.entity_ids():
        var node := _bridge.get_entity_node(entity_id) as Node3D; if node == null: continue
        if not socket: result.append({"id": entity_id, "position": node.global_position}); continue
        if not _socket_resolver.is_valid(): continue
        var socket_value: Variant = _socket_resolver.call(str(entity_id)); if not socket_value is Array: continue
        for socket_record in socket_value:
            if not socket_record is Dictionary: continue
            var socket_id: String = str(socket_record.get("socket_id", "")); if socket_id.is_empty(): continue
            var local_transform: Transform3D = _transform3d_from_dict(socket_record.get("local_transform", {}))
            result.append({"id": socket_id, "entity_id": str(entity_id), "position": (node.global_transform * local_transform).origin})
    return result
...
func _runtime_vertex_candidates() -> Array[Dictionary]:
    ...
    for entity_id in _bridge.entity_ids():
        ...
        for mesh_node in _mesh_instances(node):
            var mesh_instance := mesh_node as MeshInstance3D; if mesh_instance.mesh == null: continue
            for surface in range(mesh_instance.mesh.get_surface_count()):
                var arrays: Array = mesh_instance.mesh.surface_get_arrays(surface); if arrays.size() <= Mesh.ARRAY_VERTEX: continue
                var values: Variant = arrays[Mesh.ARRAY_VERTEX]; if not values is PackedVector3Array: continue
                var vertices: PackedVector3Array = values; var remaining: int = MAX_VERTEX_CANDIDATES_PER_ENTITY - count; if remaining <= 0: break
                var step: int = maxi(1, int(ceil(float(vertices.size()) / float(remaining))))
                for index in range(0, vertices.size(), step):
                    result.append({"id": "%s:vertex:%d" % [entity_id, count], "position": mesh_instance.global_transform * vertices[index]}); count += 1
    ...
    for mesh_node in _mesh_instances(root):
        ...
        for x in [aabb.position.x, aabb.end.x]:
            for y in [aabb.position.y, aabb.end.y]:
                for z in [aabb.position.z, aabb.end.z]: minimum = minf(minimum, (mesh_instance.global_transform * Vector3(x, y, z)).y)
...
func _sync_ghost_cell() -> Dictionary:
    ...
    var cell_id: String = _resolve_cell_id(_ghost.position); if cell_id.is_empty(): return _failure("Placement preview is outside the authored world partition.")
    var current: String = str(_ghost.get_record().get("cell_id", "")); if current == cell_id: return {"ok": true, "errors": [], "changed": false, "cell_id": cell_id}
    var result: Dictionary = _ghost.set_cell_id(cell_id); result["changed"] = result.get("ok", false); result["cell_id"] = cell_id; return result
func _resolve_cell_id(position_value: Vector3) -> String:
    if not _cell_resolver.is_valid(): return ""
    var value: Variant = _cell_resolver.call(position_value); return str(value) if value != null else ""
...
func _find_record(entity_id: String) -> Dictionary:
    ...
    return {}
func _history_failure(result: Dictionary) -> Dictionary: return _failure(str(result.get("errors", [result.get("error", "Command history operation failed.")])[0]))
...
static func _transform3d_from_dict(value: Dictionary) -> Transform3D:
    var position_value: Vector3 = _vector3(value.get("position", [0.0, 0.0, 0.0])); var rotation_value: Vector3 = _vector3(value.get("rotation_degrees", [0.0, 0.0, 0.0])) * (PI / 180.0); var scale_value: Vector3 = _vector3(value.get("scale", [1.0, 1.0, 1.0]))
