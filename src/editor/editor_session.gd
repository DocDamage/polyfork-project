...
func bind_project(project, dirty_callback: Callable) -> Dictionary:
    if project == null:
        return _failure("Editor session requires an active project.")
...
func refresh_runtime(preserve_selection: bool = true) -> Dictionary:
    ...
    if preserve_selection and not selected.is_empty():
        ...
        if not surviving.is_empty():
            _selection.set_selected(surviving, primary)
    return result
...
func snap_selection_to_surface(hit_position: Vector3, hit_normal: Vector3) -> Dictionary:
    return move_selection_to(_snapping.snap_to_surface(hit_position, hit_normal))


func snap_selection_to_object(candidates: Array) -> Dictionary:
    return _snap_selection_to_candidates(candidates, false)


func snap_selection_to_socket(candidates: Array) -> Dictionary:
    return _snap_selection_to_candidates(candidates, true)
...
func select_entity(entity_id: String) -> Dictionary:
    return _selection.select_single(entity_id)


func toggle_entity(entity_id: String) -> Dictionary:
    return _selection.toggle_entity(entity_id)


func select_runtime_node(node: Node, additive: bool = false) -> Dictionary:
    return _selection.select_node(node, additive)
...
func set_tool(mode: StringName) -> Dictionary:
    return _gizmo.set_mode(mode)


func set_axis(axis: StringName) -> Dictionary:
    return _gizmo.set_axis(axis)


func set_snap_enabled(mode: StringName, enabled: bool) -> Dictionary:
    return _snapping.set_mode_enabled(mode, enabled)


func is_snap_enabled(mode: StringName) -> bool:
...
func _can_mutate() -> bool:
    return _project != null and _dirty_callback.is_valid()
...
func _history_failure(result: Dictionary) -> Dictionary:
    return _failure(str(result.get("error", "Editor command failed.")))
