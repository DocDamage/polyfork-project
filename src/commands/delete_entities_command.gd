...
func execute() -> bool:
    ...
    if not errors.is_empty():
        ...
        _set_error("Delete would invalidate the project: %s" % str(errors))
        return false
    return true
...
func undo() -> bool:
    ...
    if not _captured:
        ...
        return false
    var original: Array[Dictionary] = _copy_records(_project.entity_records)
    var restored: Array[Dictionary] = _copy_records(_project.entity_records)
    for entry in _captured_records:
        var index: int = mini(int(entry["index"]), restored.size())
        restored.insert(index, entry["record"].duplicate(true))
    _project.entity_records = restored
    var errors: Array[String] = _project.validate()
    if not errors.is_empty():
        _project.entity_records = original
        _set_error("Delete undo would invalidate the project: %s" % str(errors))
        return false
    return true
...
func _capture_delete_closure() -> bool:
    ...
    while changed:
        ...
        for record in _project.entity_records:
            ...
            if parent == null:
                continue
            var parent_id: String = str(parent)
