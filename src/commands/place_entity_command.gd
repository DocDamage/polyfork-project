...
func execute() -> bool:
    ...
    if not errors.is_empty():
        _set_error("Placement entity is invalid: %s" % str(errors))
        return false
    var entity_id: String = str(_record.get("entity_id", ""))
    if _find_index(entity_id) >= 0:
        _set_error("Placement entity ID is already present in the project.")
        return false
    var cell_id: String = str(_record.get("cell_id", ""))
    _created_cell_id = ""
    ...
    if not project_errors.is_empty():
        ...
        _set_error("Placement would invalidate the project: %s" % str(project_errors))
        return false
    return true
...
func undo() -> bool:
    ...
    if _project == null:
        ...
        return false
    var index: int = _find_index(str(_record.get("entity_id", "")))
    if index < 0:
    ...
    var removed_cell: bool = _remove_created_cell_if_unused()
    var project_errors: Array[String] = _project.validate()
    if not project_errors.is_empty():
        if removed_cell:
            _project.cell_ids.append(_created_cell_id)
        _project.entity_records.insert(index, removed)
        _set_error("Placement undo would invalidate the project: %s" % str(project_errors))
        return false
    return true
...
func _remove_created_cell_if_unused() -> bool:
    ...
    for record in _project.entity_records:
        if str(record.get("cell_id", "")) == _created_cell_id:
            return false
    var index: int = _project.cell_ids.find(_created_cell_id)
    if index >= 0:
