...
func bind_state(state) -> Dictionary:
    if state == null:
        return _failure("Terrain runtime requires a terrain state.")
    clear_runtime()
    _state = state
...
func update_focus(focus_position: Vector3) -> Dictionary:
    if _state == null:
        return _failure("Terrain runtime is not bound.")
    ...
    for cell_id in target_ids:
        target_set[cell_id] = true
    var loaded: Array[String] = []
    var unloaded: Array[String] = []
    for cell_id_value in _loaded_chunks.keys().duplicate():
        var cell_id := str(cell_id_value)
        if target_set.has(cell_id):
            continue
        if _state.is_cell_dirty(cell_id):
            _blocked_dirty[cell_id] = true
            continue
        _unload_chunk(cell_id)
        unloaded.append(cell_id)
    for cell_id in target_ids:
        if _loaded_chunks.has(cell_id):
            _blocked_dirty.erase(cell_id)
            continue
    ...
    var changed := not loaded.is_empty() or not unloaded.is_empty() or blocked != previous_blocked
    if changed:
        streaming_changed.emit(loaded.duplicate(), unloaded.duplicate(), blocked.duplicate())
    return {
        "ok": true,
        "errors": [],
        "changed": changed,
        "active": get_loaded_cell_ids(),
        "loaded": loaded,
        "unloaded": unloaded,
...
func sample_height(world_position: Vector3) -> float:
    ...
    if cell.is_empty():
        return 0.0
    var origin := _state.cell_origin(cell_id)
    var size := float(_state.manifest.get("cell_size", 256.0))
...
func get_blocked_dirty_cell_ids() -> Array[String]:
    var result: Array[String] = []
    for key in _blocked_dirty.keys():
        result.append(str(key))
    result.sort()
    return result


func get_chunk(cell_id: String):
    return _loaded_chunks.get(cell_id)
...
func clear_runtime() -> void:
    for cell_id_value in _loaded_chunks.keys().duplicate():
        _unload_chunk(str(cell_id_value))
...
func _load_chunk(cell_id: String) -> Dictionary:
    var cell: Dictionary = _state.get_cell(cell_id)
    if cell.is_empty():
        return _failure("Cannot stream missing terrain cell: %s" % cell_id)
