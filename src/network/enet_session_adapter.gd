...
var _role := Contract.ROLE_OFFLINE
var _session_id := ""
...
func start_session(config: Dictionary) -> Dictionary:
    ...
    match _role:
        Contract.ROLE_OFFLINE: return _start_offline()
        Contract.ROLE_HOST: return _start_host()
        Contract.ROLE_CLIENT: return _start_client()
        _: return _record_failure(["Unsupported network role: %s" % _role])

func shutdown(reason: String = "shutdown") -> Dictionary:
    var changed := _peer != null or _ready or _registry.is_active()
    if _peer != null:
...
func poll_once() -> void:
    ...
    while _peer.get_available_packet_count() > 0:
        ...
        if message.is_empty():
            ...
            continue
        var envelope_errors := Contract.validate_envelope(message)
...
func send_message(message: Dictionary, target_peer: int = MultiplayerPeer.TARGET_PEER_BROADCAST, reliable: bool = true) -> Dictionary:
    ...
    var errors := Contract.validate_envelope(message)
    ...
    var result := _peer.put_packet(Contract.encode(message))
...
func is_session_ready() -> bool: return _ready
...
func _start_offline() -> Dictionary:
    ...
    var begin_result := _registry.begin_session(_session_id)
    if not begin_result.get("ok", false): return begin_result
    var identity_result := _registry.register_peer(1, str(_config.get("player_label", "Player")), str(_config.get("team_id", "")))
...
func _start_host() -> Dictionary:
    ...
    var result := _peer.create_server(int(_config.get("port", Contract.DEFAULT_PORT)), int(_config.get("max_players", Contract.DEFAULT_MAX_PLAYERS)) - 1)
    if result != OK:
        _peer = null
    ...
    var begin_result := _registry.begin_session(_session_id)
    if not begin_result.get("ok", false): return begin_result
    var identity_result := _registry.register_peer(1, str(_config.get("player_label", "Host")), str(_config.get("team_id", "")))
    if not identity_result.get("ok", false): return identity_result
    _local_peer_id = 1
    _ready = true
    session_ready.emit(_role, _session_id, _local_peer_id)
...
func _start_client() -> Dictionary:
    _peer = ENetMultiplayerPeer.new()
    var result := _peer.create_client(str(_config.get("address", Contract.DEFAULT_ADDRESS)), int(_config.get("port", Contract.DEFAULT_PORT)))
    if result != OK:
        _peer = null
...
func _on_peer_connected(peer_id: int) -> void:
    ...
    if _role == Contract.ROLE_CLIENT and peer_id == MultiplayerPeer.TARGET_PEER_SERVER:
        var hello := Contract.make_hello(str(_config.get("project_id", "")), str(_config.get("player_label", "Player")), str(_config.get("team_id", "")))
        var result := send_message(hello, MultiplayerPeer.TARGET_PEER_SERVER, true)
        if not result.get("ok", false): _emit_error(result.get("errors", []))

func _on_peer_disconnected(peer_id: int) -> void:
    _pending_client_peers.erase(peer_id)
    var removed := _registry.remove_peer(peer_id)
...
func _handle_host_hello(sender: int, message: Dictionary) -> void:
    ...
    var hello_errors := Contract.validate_hello(message, str(_config.get("project_id", "")))
    if not hello_errors.is_empty():
        ...
        return
    var payload: Dictionary = message.get("payload", {})
    var register_result := _registry.register_peer(sender, str(payload.get("player_label", "Player")), str(payload.get("team_id", "")))
    if not register_result.get("ok", false):
        send_message(Contract.make_reject(register_result.get("errors", [])), sender, true)
        _peer.disconnect_peer(sender)
        return
    _pending_client_peers.erase(sender)
    var identity: Dictionary = register_result.get("identity", {})
    var accept := Contract.make_accept(_session_id, sender, identity, _registry.snapshot())
    var accept_result := send_message(accept, sender, true)
    if not accept_result.get("ok", false):
        _registry.remove_peer(sender)
        _emit_error(accept_result.get("errors", []))
        return
...
func _handle_client_accept(message: Dictionary) -> void:
    ...
    if not begin_result.get("ok", false):
        _emit_error(begin_result.get("errors", [])); return
    var peers: Array = payload.get("peers", [])
    for value in peers:
        if not value is Dictionary: continue
        var identity: Dictionary = value
        var register_result := _registry.register_peer(
        ...
        if not register_result.get("ok", false):
            _emit_error(register_result.get("errors", [])); return
    _ready = true
    session_ready.emit(_role, _session_id, _local_peer_id)
...
func _apply_peer_join_message(message: Dictionary) -> void:
    ...
    var result := _registry.register_peer(
        peer_id,
        str(identity.get("player_label", "Player")),
        str(identity.get("team_id", "")),
        str(identity.get("authored_entity_id", "")),
