...
func _init(project, gameplay_state, repository, before_project: Dictionary, after_project: Dictionary, before_state: Dictionary, after_state: Dictionary, sections: Array[String]) -> void:
    _project = project
    _state = gameplay_state
    _repository = repository
    _before_project = before_project.duplicate(true)
    _after_project = after_project.duplicate(true)
    _before_state = before_state.duplicate(true)
    _after_state = after_state.duplicate(true)
    _sections = sections.duplicate()


func execute() -> bool:
    _clear_error()
    return _apply(_after_project, _after_state, _before_project, _before_state, "Gameplay edit")


func undo() -> bool:
    _clear_error()
    return _apply(_before_project, _before_state, _after_project, _after_state, "Gameplay undo")
...
func _apply(project_snapshot: Dictionary, state_snapshot: Dictionary, rollback_project: Dictionary, rollback_state: Dictionary, label: String) -> bool:
    if _project == null or _state == null or _repository == null:
        _set_error("%s requires a bound project, gameplay state, and repository." % label)
        return false
    _assign_project(project_snapshot)
    _assign_state(state_snapshot)
    var project_errors: Array[String] = _project.validate()
    var gameplay_errors: Array[String] = _state.validate(_project)
    if not project_errors.is_empty() or not gameplay_errors.is_empty():
        ...
        _set_error("%s produced invalid authored state: %s %s" % [label, project_errors, gameplay_errors])
        return false
    var persist: Dictionary = _repository.flush_sections(_state, _sections)
    if not persist.get("ok", false):
        _assign_project(rollback_project); _assign_state(rollback_state)
        var restore: Dictionary = _repository.flush_sections(_state, _sections)
        var suffix := ""
        if not restore.get("ok", false): suffix = " Recovery persistence also failed: %s" % [restore.get("errors", [])]
        _set_error("%s persistence failed: %s.%s" % [label, persist.get("errors", []), suffix])
        return false
    return true
...
func _assign_state(snapshot: Dictionary) -> void:
    for section in ["definitions", "instances", "archetypes", "prefabs", "sockets", "attachments", "prefab_instances"]:
        _state.set(section, _dictionary_array(snapshot.get(section, [])))


static func _dictionary_array(value: Array) -> Array[Dictionary]:
    var result: Array[Dictionary] = []
    for record in value: result.append(record.duplicate(true))
    return result
