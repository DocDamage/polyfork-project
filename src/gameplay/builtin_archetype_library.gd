...
static func definitions() -> Array[Dictionary]:
    return [
        _arch("door", "Door", ["door", "interactable", "collision", "save_state"], {Components.id_for("door"): {"open_angle": 90.0}}, ["interaction", "architecture"]),
        _arch("loot_container", "Loot Container", ["inventory_container", "interactable", "collision", "save_state"], {Components.id_for("inventory_container"): {"capacity": 24}}, ["inventory", "interaction"]),
        _arch("npc", "NPC", ["character_controller", "npc_brain", "dialogue_participant", "save_state"], {}, ["character", "dialogue"]),
        _arch("enemy", "Enemy", ["character_controller", "npc_brain", "health", "damageable", "save_state"], {Components.id_for("health"): {"max_health": 100.0, "current_health": 100.0}}, ["character", "combat"]),
        ...
        _arch("player", "Player", ["character_controller", "spawn_point"], {Components.id_for("character_controller"): {"move_speed": 6.0, "jump_enabled": true, "jump_strength": 6.0}, Components.id_for("spawn_point"): {"spawn_tag": "player", "enabled": true}}, ["character", "player", "instant_play"])
    ]
...
static func _arch(key: String, display_name: String, component_keys: Array, defaults: Dictionary, tags: Array) -> Dictionary:
    ...
    return {"document_type": Contracts.ARCHETYPE, "schema_version": Contracts.SCHEMA_VERSION, "archetype_id": IDS[key], "key": key, "display_name": display_name, "description": "%s archetype preset." % display_name, "required_definition_ids": required, "component_defaults": defaults.duplicate(true), "tags": tags.duplicate()}
