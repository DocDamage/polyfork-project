...
func _activate_project(project) -> bool:
    ...
    var cell_resolver := Callable(); var terrain_controller = _terrain_workspace.get_controller()
    if terrain_controller != null and terrain_controller.get_state() != null: cell_resolver = Callable(terrain_controller.get_state(), "cell_id_at_position")
    ...
    var template_result := _prepare_template_baseline(project, editor_session, cell_resolver)
    if not template_result.get("ok", false): return _activation_failure("Unable to initialize project template", template_result)
    if workspace_screen.has_method("get_play_session") and terrain_controller != null: workspace_screen.get_play_session().configure_streaming(Callable(terrain_controller, "update_streaming_focus"))
    return true
...
func _prepare_template_baseline(project, editor_session, cell_resolver: Callable) -> Dictionary:
    ...
    if project.runtime_config.is_empty() or str(project.runtime_config.get("template_id", "")) != project.template_id:
        ...
        changed = true
    var gameplay_service = _gameplay_workspace.get_service(); var starter_result: Dictionary = application.materialize_starters(project, manifest, editor_session, gameplay_service, cell_resolver)
