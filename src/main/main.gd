...
func get_terrain_workspace():
    return _terrain_workspace


func get_gameplay_workspace():
    return _gameplay_workspace


func get_visual_scripting_workspace():
    return _visual_scripting_workspace
...
func _activate_project(project) -> bool:
    ...
    var terrain_result: Dictionary = _terrain_workspace.bind_project(
        project,
        project_directory,
        editor_session,
        Callable(self, "mark_project_dirty")
    ...
    var gameplay_result: Dictionary = _gameplay_workspace.bind_project(
        project,
        project_directory,
        editor_session,
        Callable(self, "mark_project_dirty"),
    ...
    var visual_result: Dictionary = _visual_scripting_workspace.bind_project(
        project,
        project_directory,
        editor_session,
        Callable(self, "mark_project_dirty")
    ...
    if workspace_screen.has_method("get_play_session"):
        ...
        play_session.configure_visual_graph_provider(
            Callable(_visual_scripting_workspace.get_service(), "get_graphs")
        )
...
func _prepare_template_baseline(project, editor_session, cell_resolver: Callable) -> Dictionary:
    ...
    var starter_result: Dictionary = application.materialize_starters(
        project,
        manifest,
        editor_session,
        gameplay_service,
    ...
    return {
        ...
        "errors": [],
        "changed": true,
        "starter_entity_ids": starter_result.get("starter_entity_ids", {})
    }
...
func _on_workspace_mode_changed(mode: StringName) -> void:
    if _autosave_service != null:
        _autosave_service.set_suspended(mode == &"play")
    if mode == &"play":
        _close_contextual_tools()


func _ensure_build_mode() -> void:
    if workspace_screen == null or not workspace_screen.has_method("get_mode"):
        return
    if workspace_screen.get_mode() != &"play":
        return
    var switch = workspace_screen.get_node_or_null("TopBar/TopMargin/TopRow/ModeSlot/ModeSwitch")
    if switch != null and switch.has_method("set_mode"):
        switch.set_mode(&"build")


func _close_contextual_tools() -> void:
    if _visual_scripting_workspace != null:
        _visual_scripting_workspace.close_tool()
    if _gameplay_workspace != null:
        _gameplay_workspace.close_tool()
    if _terrain_workspace != null:
        _terrain_workspace.close_tool()
...
func _activation_failure(message: String, result: Dictionary) -> bool:
    ...
    if _autosave_service != null:
        _autosave_service.detach_project()
    return false


func _on_workspace_status(message: String, _is_error: bool) -> void:
    var status_label := workspace_screen.get_node_or_null(
        "StatusBar/StatusMargin/StatusRow/StatusState"
    ) as Label
