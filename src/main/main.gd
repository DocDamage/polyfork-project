...
func _unhandled_input(event: InputEvent) -> void:
    ...
    if workspace_screen.visible:
        if _ai_workspace != null and _ai_workspace.handle_cancel():
...
func _attach_workspace_layers() -> void:
    ...
    _ai_workspace = AiWorkspaceLayer.new()
    workspace_screen.add_child(_ai_workspace)
...
func _activate_project(project) -> bool:
    ...
    var procedural_result: Dictionary = _procedural_workspace.bind_project(project, project_directory, editor_session, Callable(self, "mark_project_dirty"), terrain_controller, workspace_screen.get_asset_library(), _gameplay_workspace.get_service())
    ...
    var ai_result: Dictionary = _ai_workspace.bind_project(
        project,
        project_directory,
        editor_session,
        ...
        _visual_scripting_workspace.get_service(),
        _procedural_workspace.get_service(),
        _procedural_workspace.get_runtime(),
        _environment_workspace.get_service()
    )
