...
func has_current_thumbnail(record: Dictionary) -> bool:
    var path: String = _current_path(record)
...
func ensure_thumbnail(record: Dictionary, preview_node: Node = null, fallback_reason: String = "") -> Dictionary:
    var asset_id: String = str(record.get("asset_id", "")); var content_hash: String = str(record.get("content_hash", ""))
    ...
    var make_error: Error = DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path(cache_root))
    if make_error != OK: return _failure("Unable to create thumbnail cache directory: %s" % make_error)
    var cache_key: String = _cache_key(asset_id, content_hash); var path: String = cache_root.path_join(cache_key + ".png")
    if FileAccess.file_exists(path):
        var existing_kind: String = str(record.get("thumbnail", {}).get("kind", "cached"))
    ...
    var result: Dictionary = _render_geometry(preview_node) if preview_node != null else {"ok": false, "image": null, "vertex_count": 0}
    var image: Image
    var kind: String = "geometry_projection"
    var depicts: bool = true
    if result.get("ok", false): image = result["image"] as Image
    else:
    ...
    var thumbnail: Dictionary = _thumbnail_record(cache_key, content_hash, path, kind, depicts)
...
func _render_geometry(preview_node: Node) -> Dictionary:
    ...
    for vertex in vertices:
        var point: Vector2 = Vector2(vertex.x - vertex.z * 0.55, -vertex.y + (vertex.x + vertex.z) * 0.18)
    ...
    var image: Image = Image.create(WIDTH, HEIGHT, false, Image.FORMAT_RGBA8); image.fill(Color(0.035, 0.055, 0.075, 1.0))
    var scale_value: float = min(float(WIDTH - 30) / span.x, float(HEIGHT - 24) / span.y)
    var used: Vector2 = span * scale_value
    var offset: Vector2 = Vector2((WIDTH - used.x) * 0.5, (HEIGHT - used.y) * 0.5)
    var accent: Color = Color(0.52, 0.86, 0.42, 1.0); var shadow: Color = Color(0.20, 0.34, 0.28, 1.0)
    for index in range(projected.size()):
        var p: Vector2 = (projected[index] - low) * scale_value + offset
        _draw_dot(image, Vector2i(roundi(p.x), roundi(p.y)), accent)
        if index > 0 and index % 3 != 0:
            var previous: Vector2 = (projected[index - 1] - low) * scale_value + offset
    ...
    if node is MeshInstance3D:
        ...
        if mesh != null:
            for surface in range(mesh.get_surface_count()):
                ...
                var mesh_vertices: PackedVector3Array = values
                var remaining: int = maxi(1, MAX_VERTICES - output.size())
                var step: int = maxi(1, int(ceil(float(mesh_vertices.size()) / float(remaining))))
                for index in range(0, mesh_vertices.size(), step):
                    output.append(local_transform * mesh_vertices[index])
                    if output.size() >= MAX_VERTICES: return
...
func _fallback_image() -> Image:
    var image: Image = Image.create(WIDTH, HEIGHT, false, Image.FORMAT_RGBA8); image.fill(Color(0.06, 0.075, 0.095, 1.0))
    var outline: Color = Color(0.35, 0.42, 0.50, 1.0)
...
func _invalidate_asset(asset_id: String) -> void:
    ...
    directory.list_dir_begin(); var entry: String = directory.get_next()
...
static func _draw_line(image: Image, start: Vector2i, finish: Vector2i, color: Color) -> void:
    var x0: int = start.x; var y0: int = start.y; var x1: int = finish.x; var y1: int = finish.y
    var dx: int = absi(x1 - x0); var sx: int = 1 if x0 < x1 else -1; var dy: int = -absi(y1 - y0); var sy: int = 1 if y0 < y1 else -1; var error: int = dx + dy
    while true:
