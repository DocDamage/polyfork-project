...
func _run_multiplayer_smoke() -> void:
    ...
    if match_snapshot.get("players", []).size() < expected_peers:
        ...
        get_tree().quit(1); return
    print("PASS: Phase 15 standalone multiplayer %s connected. peers=%d local_input=true remote_input=false keyboard_mouse=true gamepad=true" % [str(final_status.get("role", "unknown")), int(final_status.get("peer_count", 0))])
    get_tree().quit(0)
