...
func _ready() -> void:
    ...
    if not result.get("ok", false): push_error("Standalone runtime failed: %s" % str(result.get("errors", []))); get_tree().quit(1); return
    var input_result: Dictionary = _verify_input_contract()
    if not input_result.get("ok", false): push_error("Standalone runtime input verification failed: %s" % str(input_result.get("errors", []))); get_tree().quit(1); return
    ...
    if OS.get_environment("POLYFORK_MULTIPLAYER_SMOKE") == "1": call_deferred("_run_multiplayer_smoke"); return
    if OS.get_environment("POLYFORK_EXPORT_SMOKE") == "1": print("PASS: Phase 13 standalone export runtime smoke completed."); get_tree().quit(0)
...
func start_runtime() -> Dictionary:
    ...
    _assets = AssetResolver.new(); var asset_bind: Dictionary = _assets.bind_manifest(manifest_read.get("data", {}), _bundle.get("gameplay_state")); if not asset_bind.get("ok", false): return asset_bind
    _adapter = SessionAdapter.new(); add_child(_adapter); var adapter_bind: Dictionary = _adapter.bind_project(_bundle.get("project_data", {}), Callable(_assets, "instantiate_asset_scene")); if not adapter_bind.get("ok", false): return adapter_bind
    _terrain_runtime = TerrainRuntime.new(); add_child(_terrain_runtime); var terrain_bind: Dictionary = _terrain_runtime.bind_state(_bundle.get("terrain_state")); if not terrain_bind.get("ok", false): return terrain_bind
    var initial_cells: Array[String] = _terrain_runtime.get_loaded_cell_ids(); var entity_stream: Dictionary = _adapter.set_active_cell_ids(initial_cells); if not entity_stream.get("ok", false): return entity_stream
    _procedural_runtime = ProceduralRuntime.new(); add_child(_procedural_runtime); var procedural_bind: Dictionary = _procedural_runtime.bind_state(_bundle.get("procedural_state"), _bundle.get("terrain_state"), _terrain_runtime, _assets); if not procedural_bind.get("ok", false): return procedural_bind
    var profile_value: Variant = _bundle.get("performance_profile", {})
    if profile_value is Dictionary and not profile_value.is_empty():
        var quality_result: Dictionary = _procedural_runtime.configure_performance_profile(profile_value); if not quality_result.get("ok", false): return quality_result
    _play_session = PlaySession.new(); add_child(_play_session)
    if profile_value is Dictionary and not profile_value.is_empty():
        var play_profile_result: Dictionary = _play_session.configure_performance_profile(profile_value); if not play_profile_result.get("ok", false): return play_profile_result
    _play_session.configure_project_directory("user://polyfork_runtime/%s" % str(_bundle.get("project_data", {}).get("project_id", "unknown")))
    _play_session.configure_visual_graph_provider(Callable(self, "_visual_graphs")); _play_session.configure_gameplay_state_provider(Callable(self, "_gameplay_snapshot")); _play_session.configure_environment_state_provider(Callable(self, "_environment_bundle")); _play_session.configure_streaming(Callable(self, "_update_streaming")); _play_session.exit_requested.connect(_on_exit_requested)
    ...
    if not multiplayer_result.get("ok", false): _play_session.exit_play(); return multiplayer_result
    play_result["multiplayer"] = multiplayer_result; return play_result
...
func _start_multiplayer_from_environment() -> Dictionary:
    ...
    var project_data: Dictionary = _bundle.get("project_data", {}); var runtime: Dictionary = project_data.get("runtime", {}) if project_data.get("runtime", {}) is Dictionary else {}; var capability: Dictionary = runtime.get("multiplayer", {}) if runtime.get("multiplayer", {}) is Dictionary else {}
    if not bool(capability.get("enabled", false)): return _failure("This exported project does not declare multiplayer capability.")
    var runtime_path: String = "res://src/network/" + "network_runtime_service.gd"; var runtime_script: Script = load(runtime_path)
    if runtime_script == null: return _failure("Multiplayer runtime dependency closure could not be loaded from this export.")
    _network_runtime = runtime_script.new(); _network_runtime.name = "StandaloneNetworkRuntime"; add_child(_network_runtime)
    var port_text: String = OS.get_environment("POLYFORK_MULTIPLAYER_PORT").strip_edges(); var port: int = int(port_text) if not port_text.is_empty() else 24815; var player_label: String = OS.get_environment("POLYFORK_MULTIPLAYER_PLAYER").strip_edges()
    ...
    if role == "host": result = _network_runtime.host(port, player_label)
        ...
        var address: String = OS.get_environment("POLYFORK_MULTIPLAYER_ADDRESS").strip_edges(); if address.is_empty(): address = "127.0.0.1"; result = _network_runtime.join(address, port, player_label)
    if not result.get("ok", false): return result
    return {"ok": true, "errors": [], "active": true, "role": role, "port": port}

func _run_multiplayer_smoke() -> void:
    if _network_runtime == null: push_error("Phase 15 multiplayer smoke requested without an active network runtime."); get_tree().quit(1); return
    var expected_text: String = OS.get_environment("POLYFORK_MULTIPLAYER_EXPECTED_PEERS").strip_edges(); var expected_peers: int = maxi(2, int(expected_text) if not expected_text.is_empty() else 2); var ready := false; var remote_input_disabled := false
    for _index in range(900):
        ...
        for identity in status.get("peers", []):
            ...
            if remote != null and is_instance_valid(remote): remote_input_disabled = not remote.has_method("is_local_input_enabled") or not bool(remote.is_local_input_enabled()); if remote_input_disabled: break
        var local_player = _play_session.get_player(); var local_input_ok: bool = local_player != null and (not local_player.has_method("is_local_input_enabled") or bool(local_player.is_local_input_enabled()))
        if local_input_ok and remote_input_disabled: ready = true; break
    if not ready: push_error("Phase 15 standalone multiplayer smoke did not reach converged two-peer player replication."); get_tree().quit(1); return
    var final_status: Dictionary = _network_runtime.get_status(); var match_snapshot: Dictionary = _network_runtime.get_match_snapshot()
    if match_snapshot.get("players", []).size() < expected_peers: push_error("Phase 15 standalone multiplayer smoke did not converge match membership."); get_tree().quit(1); return
...
func _update_streaming(position_value: Vector3) -> Dictionary:
    var terrain_result: Dictionary = _terrain_runtime.update_focus(position_value); if not terrain_result.get("ok", false): return terrain_result
    var entity_result: Dictionary = _adapter.set_active_cell_ids(_terrain_runtime.get_loaded_cell_ids()); if not entity_result.get("ok", false): return entity_result
