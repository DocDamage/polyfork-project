...
func configure_performance_profile(profile: Dictionary) -> Dictionary:
    ...
    for batch_value in _batches.values():
        if batch_value != null and is_instance_valid(batch_value) and batch_value.has_method("configure_quality"):
            batch_value.configure_quality(_performance_profile)
            configured += 1
...
func set_active_cell_ids(cell_ids: Array[String]) -> Dictionary:
    ...
    next_ids.sort()
    if next_ids == _active_cell_ids:
        return {"ok": true, "errors": [], "changed": false, "active_cell_ids": _active_cell_ids.duplicate(), "batch_count": batch_count(), "instance_count": total_instance_count(), "spline_node_count": spline_node_count()}
...
func set_environment_wind(wind_state: Dictionary) -> Dictionary:
    var direction_value: Variant = wind_state.get("direction", Vector3.RIGHT)
    var direction := Vector3.RIGHT
    if direction_value is Vector3:
        direction = direction_value
    elif direction_value is Array and direction_value.size() == 3:
        direction = Vector3(float(direction_value[0]), float(direction_value[1]), float(direction_value[2]))
    else:
        return _failure("Procedural environment wind direction is invalid.")
    if direction.length_squared() > 0.000001: direction = direction.normalized()
    var speed := float(wind_state.get("speed_mps", 0.0))
    var gust := float(wind_state.get("gust_strength", 0.0))
    ...
    wind_changed.emit(_environment_wind.duplicate(true))
    return {"ok": true, "errors": [], "consumed": true, "batch_count": batch_count(), "spline_node_count": spline_node_count()}

func get_environment_wind() -> Dictionary:
    return _environment_wind.duplicate(true)
...
func _bind_scale_policy() -> void:
    ...
    var callback := Callable(self, "_on_scale_preferences_changed")
