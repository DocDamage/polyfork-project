...
func generate_for_cell(layer: Dictionary, foliage_set: Dictionary, cell_id: String, terrain_state, terrain_runtime) -> Dictionary:
    ...
    for value in layer.get("strokes", []):
        ...
        if str(stroke.get("cell_id", "")) != cell_id:
            continue
        if str(stroke.get("operation", "")) == "paint": paints.append(stroke)
        elif str(stroke.get("operation", "")) == "erase": erases.append(stroke)
    paints.sort_custom(func(a: Dictionary, b: Dictionary) -> bool: return str(a.get("stroke_id", "")) < str(b.get("stroke_id", "")))
    erases.sort_custom(func(a: Dictionary, b: Dictionary) -> bool: return str(a.get("stroke_id", "")) < str(b.get("stroke_id", "")))
    ...
    for stroke in paints:
        if transforms.size() >= max_instances: break
        ...
        for _attempt in range(attempts):
            if produced >= requested or transforms.size() >= max_instances: break
            ...
            if terrain_state.cell_id_at_position(candidate) != cell_id: continue
            var height: float = terrain_runtime.sample_height(candidate)
            if height < float(height_range[0]) or height > float(height_range[1]): continue
            candidate.y = height
            if _is_erased(candidate, erases): continue
            if minimum_spacing > 0.0 and _too_close(candidate, accepted_positions, minimum_spacing): continue
            ...
            if slope < float(slope_range[0]) or slope > float(slope_range[1]): continue
            ...
            if bool(foliage_set.get("align_to_normal", true)): basis = Basis(Quaternion(Vector3.UP, normal)) * basis
            basis = basis.scaled(Vector3.ONE * scale_value)
            var origin: Vector3 = candidate + normal * (_primitive_ground_offset(foliage_set) * scale_value)
            transforms.append(Transform3D(basis, origin))
            accepted_positions.append(candidate)
            produced += 1
    return {"ok": true, "errors": [], "transforms": transforms, "instance_count": transforms.size(), "cell_id": cell_id, "scatter_layer_id": str(layer.get("scatter_layer_id", ""))}


static func _primitive_ground_offset(foliage_set: Dictionary) -> float:
    var source: Variant = foliage_set.get("source", {})
    if not source is Dictionary or str(source.get("kind", "")) != "primitive": return 0.0
    match str(source.get("primitive", "")):
        "grass": return 0.525
        "shrub": return 0.675
        "tree": return 1.8
        "post": return 0.9
...
static func _is_erased(position_value: Vector3, erases: Array[Dictionary]) -> bool:
    for stroke in erases:
        ...
        if flat_delta.length() <= effective_radius: return true
    return false
...
static func _too_close(position_value: Vector3, accepted: Array[Vector3], minimum_spacing: float) -> bool:
    ...
    for other in accepted:
        ...
        if dx * dx + dz * dz < minimum_squared: return true
    return false
...
static func _terrain_normal(position_value: Vector3, terrain_runtime) -> Vector3:
    ...
    if normal.dot(Vector3.UP) < 0.0: normal = -normal
    return Vector3.UP if normal.length_squared() < 0.5 else normal


static func _vector3(value: Variant) -> Vector3:
    if not value is Array or value.size() != 3: return Vector3.ZERO
