...
const VisualGraphDebugToolbar = preload("res://src/app/workspace/visual_graph_debug_toolbar.gd")
...
func _ready() -> void:
    ...
    _panel = VisualGraphPanel.new(); add_child(_panel); _panel.close_requested.connect(close_tool); _panel.status_changed.connect(func(message: String, is_error: bool): status_changed.emit(message, is_error))
    _debug_toolbar = VisualGraphDebugToolbar.new(); add_child(_debug_toolbar); _debug_toolbar.status_changed.connect(func(message: String, is_error: bool): status_changed.emit(message, is_error)); set_process_unhandled_input(true)
...
func bind_project(project, project_directory: String, editor_session, dirty_callback: Callable) -> Dictionary:
    ...
    _panel.bind_service(_service); _debug_toolbar.bind_service(_service, _panel); _bound = true; close_tool()
...
func open_tool() -> void:
    ...
    _panel.open_panel(); _debug_toolbar.open_toolbar(); open_changed.emit(true)
func close_tool() -> void:
    if _panel != null: _panel.close_panel()
    if _debug_toolbar != null: _debug_toolbar.close_toolbar()
...
func get_debug_toolbar(): return _debug_toolbar
