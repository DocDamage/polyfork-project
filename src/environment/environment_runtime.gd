...
func advance(delta: float, focus_position: Vector3 = Vector3.ZERO) -> Dictionary:
    ...
    if _play_mode and bool(authored.get("progress_time_in_play", true)):
        var day_length: float = maxf(0.001, float(authored.get("day_length_seconds", 1200.0)))
        _time_hours = fposmod(_time_hours + maxf(0.0, delta) * 24.0 / day_length, 24.0)
    return _evaluate(focus_position, maxf(0.0, delta), false)
...
func set_time_of_day(hours: float) -> Dictionary:
    ...
    _time_hours = hours
    var result: Dictionary = _evaluate(_last_focus, 0.0, true)
    if result.get("ok", false): environment_event.emit("environment.time_changed", {"time_of_day_hours": _time_hours})
    return result

func set_weather_profile(profile_id: String, transition_seconds: float = -1.0) -> Dictionary:
    if _get_profile(profile_id).is_empty(): return _failure("Environment weather profile does not resolve: %s" % profile_id)
    _explicit_weather_id = profile_id
    _transition_duration = _default_transition_seconds() if transition_seconds < 0.0 else maxf(0.0, transition_seconds)
    var result: Dictionary = _evaluate(_last_focus, 0.0, false, true)
    if result.get("ok", false): environment_event.emit("environment.weather_changed", {"weather_profile_id": profile_id})
    return result
...
func clear_weather_override(transition_seconds: float = -1.0) -> Dictionary:
    ...
    _transition_duration = _default_transition_seconds() if transition_seconds < 0.0 else maxf(0.0, transition_seconds)
    var result: Dictionary = _evaluate(_last_focus, 0.0, false, true)
    if result.get("ok", false): environment_event.emit("environment.weather_override_cleared", {})
    return result

func set_rendering_enabled(value: bool) -> void: _render_bridge.set_rendering_enabled(value)
func is_rendering_enabled() -> bool: return _render_bridge.is_rendering_enabled()
...
func get_render_snapshot() -> Dictionary: return _render_bridge.get_snapshot()
...
func _evaluate(focus_position: Vector3, delta: float, force_immediate: bool, force_transition: bool = false) -> Dictionary:
    ...
    var profile_id: String = str(profile.get("weather_profile_id", ""))
    ...
    var profile_changed: bool = profile_id != _active_profile_id
    if (profile_changed or force_transition) and not _evaluated.is_empty() and not force_immediate:
        ...
        _transition_elapsed = 0.0
        if _transition_duration <= 0.0: _transition_duration = _default_transition_seconds()
    ...
    if not _transition_from.is_empty() and _transition_duration > 0.0:
        _transition_elapsed = minf(_transition_duration, _transition_elapsed + delta)
        var weight: float = _transition_elapsed / _transition_duration
        _evaluated = Evaluator.blend(_transition_from, target, weight)
        for key in ["cell_id", "biome_id", "weather_source", "water_hooks", "warning", "time_of_day_hours", "sun_rotation_degrees"]: _evaluated[key] = target.get(key)
        if weight >= 1.0:
            _transition_from.clear()
            _transition_duration = 0.0
    else:
...
func _resolve_profile_context(position_value: Vector3) -> Dictionary:
    _last_warning = ""
    var authored: Dictionary = _document.get("authored_state", {})
    var profile_id: String = _explicit_weather_id
    var source: String = "runtime_override" if not profile_id.is_empty() else "default"
    var cell_id: String = ""
    var biome_id: String = ""
    ...
    if _terrain_state != null and _terrain_state.has_method("get_cell_at_position"):
        ...
        if not cell.is_empty():
            ...
            if profile_id.is_empty():
                var override_profile: String = str(biome_override.get("weather_profile_id", ""))
                if not override_profile.is_empty():
                    if _get_profile(override_profile).is_empty(): _last_warning = "Biome environment override references a missing weather profile; using authored default."
                    else:
                ...
                if profile_id.is_empty() and _terrain_state.has_method("get_biome"):
                    ...
                    var terrain_profile: String = str(future_defaults.get("environment_profile_id", ""))
                    if not terrain_profile.is_empty() and terrain_profile != "<null>":
                        if _get_profile(terrain_profile).is_empty(): _last_warning = "Terrain biome references a missing environment profile; using authored default."
                        else:
                            profile_id = terrain_profile
                            source = "terrain_biome_default"
            for hook_id in biome_override.get("water_hook_ids", []):
                var hook: Dictionary = _get_water_hook(str(hook_id))
    ...
    if profile.is_empty():
        var fallback_id: String = str(authored.get("default_weather_profile_id", ""))
...
func _get_profile(profile_id: String) -> Dictionary:
    for record in _document.get("weather_profiles", []):
        if record is Dictionary and str(record.get("weather_profile_id", "")) == profile_id: return record.duplicate(true)
    return {}

func _get_biome_override(biome_id: String) -> Dictionary:
    for record in _document.get("biome_overrides", []):
        if record is Dictionary and str(record.get("biome_id", "")) == biome_id: return record.duplicate(true)
    return {}

func _get_water_hook(hook_id: String) -> Dictionary:
    for record in _document.get("water_hooks", []):
        if record is Dictionary and str(record.get("water_hook_id", "")) == hook_id: return record.duplicate(true)
    return {}

func _default_transition_seconds() -> float:
    return maxf(0.0, float(_document.get("authored_state", {}).get("default_transition_seconds", 5.0)))
