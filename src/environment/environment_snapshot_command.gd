class_name PlayWorldEnvironmentSnapshotCommand
...
func execute() -> bool:
    _clear_error()
    return _apply(_after, _before, "Environment edit")

func undo() -> bool:
    _clear_error()
    return _apply(_before, _after, "Environment undo")

func _apply(target: Dictionary, rollback: Dictionary, label: String) -> bool:
    if _project == null or _state == null or _repository == null:
        _set_error("%s requires project, environment state, and repository." % label)
        return false
    var load_errors: Array[String] = _state.replace_document(target)
    if not load_errors.is_empty():
        _set_error("%s produced invalid environment state: %s" % [label, str(load_errors)])
        return false
