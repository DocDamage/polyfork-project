...
static func evaluate(profile: Dictionary, time_hours: float, authored_state: Dictionary = {}, biome_override: Dictionary = {}) -> Dictionary:
    ...
    var local_time := fposmod(time_hours + float(biome_override.get("time_offset_hours", 0.0)), 24.0)
    var solar_angle := TAU * ((local_time - 6.0) / 24.0)
    var sun_height := sin(solar_angle)
    var daylight := clamp((sun_height + 0.12) / 0.72, 0.0, 1.0)
    var night_factor := 1.0 - daylight
    var fog_multiplier := max(0.0, float(biome_override.get("fog_density_multiplier", 1.0)))
    var wind_multiplier := max(0.0, float(biome_override.get("wind_multiplier", 1.0)))
    var fog_enabled := bool(authored_state.get("fog_enabled", true))
    var wind_enabled := bool(authored_state.get("wind_enabled", true))
    var direction := _normalized_direction(profile.get("wind_direction", [1.0, 0.0, 0.0]))
    var sun_energy := float(profile.get("sun_energy", 1.0)) * lerp(0.04, 1.0, daylight)
    var ambient_energy := float(profile.get("ambient_energy", 0.5)) * lerp(0.24, 1.0, daylight)
    return {
        "ok": true,
        "errors": [],
...
static func blend(a: Dictionary, b: Dictionary, weight: float) -> Dictionary:
    ...
    for key in ["sun_energy", "ambient_energy", "fog_density", "precipitation", "cloud_coverage", "daylight", "night_factor"]:
        result[key] = lerp(float(a.get(key, 0.0)), float(b.get(key, 0.0)), t)
    ...
    var direction: Vector3 = aw.get("direction", Vector3.RIGHT).lerp(bw.get("direction", Vector3.RIGHT), t)
    ...
    result["wind"] = {
        ...
        "direction": direction,
        "speed_mps": lerp(float(aw.get("speed_mps", 0.0)), float(bw.get("speed_mps", 0.0)), t),
        "gust_strength": lerp(float(aw.get("gust_strength", 0.0)), float(bw.get("gust_strength", 0.0)), t),
    }
