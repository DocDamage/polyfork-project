...
func stage(proposal: Dictionary) -> Dictionary:
    ...
    var changed := {"project": false, "gameplay": false, "visual": false, "procedural": false, "environment": false}
    ...
    for action_value in proposal.get("actions", []):
        ...
        var action_result: Dictionary = _stage_action(action, after_project, gameplay_stage, visual_after, procedural_after, environment_after)
        if not action_result.get("ok", false): return action_result
        var action_id: String = str(action.get("action_id", ""))
        generated[action_id] = action_result.get("generated", {}).duplicate(true)
        for key_value in action_result.get("changed", []).duplicate(): changed[str(key_value)] = true
    ...
    return {
        ...
        "gameplay_after": _gameplay_snapshot(gameplay_stage),
        "visual_before": visual_before,
        "visual_after": visual_after,
        "procedural_before": procedural_before,
        "procedural_after": procedural_after,
...
func _stage_action(action: Dictionary, project_after: Dictionary, gameplay_stage, visual_after: Array[Dictionary], procedural_after: Dictionary, environment_after: Dictionary) -> Dictionary:
    var action_type: String = str(action.get("type", ""))
    var args: Dictionary = action.get("arguments", {})
    match action_type:
        "entity.place_asset": return _stage_entity_place(args, project_after, str(args.get("asset_id", "")))
        "entity.place_proxy": return _stage_entity_place(args, project_after, "")
        "entity.transform": return _stage_entity_transform(args, project_after)
        "gameplay.add_component": return _stage_gameplay_component(args, project_after, gameplay_stage)
...
func _stage_entity_place(args: Dictionary, project_after: Dictionary, asset_id: String) -> Dictionary:
    ...
    var record: Dictionary = {
        ...
        "transform": {
            "position": _vector3_array(args.get("position", [0.0, 0.0, 0.0])),
            "rotation_degrees": _vector3_array(args.get("rotation_degrees", [0.0, 0.0, 0.0])),
            "scale": _vector3_array(args.get("scale", [1.0, 1.0, 1.0])),
        },
    }
...
func _stage_entity_transform(args: Dictionary, project_after: Dictionary) -> Dictionary:
...
func _stage_gameplay_component(args: Dictionary, project_after: Dictionary, stage) -> Dictionary:
    ...
    project_after["entities"] = entities
    return {"ok": true, "errors": [], "changed": ["project", "gameplay"], "generated": {"component_instance_ids": created}}
...
func _validate_staged(project_after: Dictionary, gameplay_stage, visual_after: Array[Dictionary], procedural_after: Dictionary, environment_after: Dictionary, changed: Dictionary) -> Array[String]:
    ...
    if bool(changed.get("visual", false)):
        ...
        if errors.is_empty():
            var compiler = preload("res://src/visual_scripting/visual_graph_compiler.gd").new()
            var compiled: Dictionary = compiler.compile_registry(visual_after)
...
func _sync_registries(project_after: Dictionary, visual_after: Array[Dictionary], procedural_after: Dictionary, environment_after: Dictionary) -> void:
    ...
    for graph in visual_after:
        if graph is Dictionary: visual_ids.append(str(graph.get("graph_id", "")))
    visual_ids.sort(); registries["visual_graph_ids"] = visual_ids
