...
func _init(settings_path_override: String = "") -> void:
    ...
    _provider_registry = ProviderRegistry.new(settings_path_override)
    add_child(_provider_registry)
    _provider_registry.status_changed.connect(func(message: String, is_error: bool) -> void: status_changed.emit(message, is_error))
...
func request(mode: String, prompt: String) -> Dictionary:
    ...
    _request["provider_id"] = str(provider_result.get("descriptor", {}).get("provider_id", ""))
    _request["provider_scope"] = str(provider_result.get("descriptor", {}).get("scope", ""))
    _request["privacy"] = provider_result.get("privacy", {}).duplicate(true)
    _tool_rounds = 0
    _last_preview.clear()
    _messages = _initial_messages(prompt, mode, _request["privacy"])
    _set_busy(true)
...
func execute_last_preview() -> Dictionary:
    ...
    var proposal: Dictionary = _last_preview.get("proposal", {})
    var preview: Dictionary = _last_preview.get("preview", {})
    if proposal.is_empty() or not preview.get("ok", false) or bool(preview.get("mutates_authored_state", true)):
        return _failure("The cached AI Preview is not executable.")
    var privacy: Dictionary = _last_preview.get("privacy", Contracts.default_privacy_policy())
    var result: Dictionary = _execution_service.execute(
        proposal,
        str(_last_preview.get("provider_id", "")),
        str(_last_preview.get("prompt", "")),
        bool(privacy.get("record_prompts", true))
    )
    if not result.get("ok", false):
        status_changed.emit(str(result.get("errors", ["AI Execute failed."])[0]), true)
        return result
    result["mode"] = "execute"
    result["proposal"] = proposal.duplicate(true)
...
func cancel() -> Dictionary:
    ...
    var result: Dictionary = _provider.cancel(str(_request.get("request_id", "")))
    _set_busy(false)
    _request.clear(); _messages.clear(); _tool_rounds = 0
    return result
...
func _submit_provider() -> Dictionary:
    return _provider.submit({
        "request_id": str(_request.get("request_id", "")),
        "messages": _messages.duplicate(true),
        "tools": _query_service.tool_definitions(),
        "temperature": 0.2,
    })
...
func _on_provider_response(request_id: String, response: Dictionary) -> void:
    ...
    if not response.get("ok", false):
        _finish_failure(str(response.get("errors", ["AI provider request failed."])[0]))
        return
    var tool_calls: Array = response.get("tool_calls", [])
    if not tool_calls.is_empty():
        if _tool_rounds >= MAX_TOOL_ROUNDS:
            _finish_failure("AI provider exceeded the bounded project-query round limit.")
            return
        _tool_rounds += 1
        ...
        for call_value in tool_calls:
            ...
            var tool_result: Dictionary = _query_service.execute_tool(
                str(call_value.get("tool", "")),
                call_value.get("arguments", {}),
                int(_request.get("privacy", {}).get("max_context_items", Contracts.DEFAULT_CONTEXT_ITEMS))
            )
            if not tool_result.get("ok", false):
                _finish_failure(str(tool_result.get("errors", ["AI query tool failed."])[0]))
                return
            _messages.append({
                "role": "tool",
                "tool_call_id": str(call_value.get("call_id", "")),
                "content": _bounded_tool_content(tool_result.get("result"), int(_request.get("privacy", {}).get("max_context_chars", Contracts.DEFAULT_CONTEXT_CHARS))),
            })
        var submit_result: Dictionary = _submit_provider()
    ...
    if not structured is Dictionary:
        _finish_failure("AI provider must return a structured JSON proposal after project queries are complete.")
        return
    ...
    if not validation.is_empty():
        _finish_failure("AI provider proposal was rejected: %s" % "; ".join(validation))
        return
    var mode: String = str(_request.get("mode", "suggest"))
    if mode == "suggest":
        _finish_success({"ok": true, "errors": [], "mode": "suggest", "proposal": proposal, "provider_id": _request.get("provider_id", ""), "provider_scope": _request.get("provider_scope", ""), "tool_rounds": _tool_rounds, "mutates_authored_state": false})
        return
    var preview: Dictionary = _preview_service.preview(proposal)
    if not preview.get("ok", false):
        _finish_failure(str(preview.get("errors", ["AI Preview validation failed."])[0]))
        return
...
func _initial_messages(prompt: String, mode: String, privacy: Dictionary) -> Array[Dictionary]:
    var max_items: int = int(privacy.get("max_context_items", Contracts.DEFAULT_CONTEXT_ITEMS))
    var max_chars: int = int(privacy.get("max_context_chars", Contracts.DEFAULT_CONTEXT_CHARS))
    var context: Dictionary = _query_service.context_snapshot(prompt, max_items)
    var context_text: String = _bounded_text(JSON.stringify(context), max_chars)
    var action_text: String = JSON.stringify(_action_registry.action_catalog())
...
func _finish_failure(message: String) -> void:
    ...
    result_ready.emit({"ok": false, "errors": [message]})
...
func _is_build_mode() -> bool:
    ...
    var value: Variant = _mode_provider.call()
    return str(value) == "build"


static func _disclosure_text(request: Dictionary) -> String:
    return "Using local AI provider; project context stays on this machine." if str(request.get("provider_scope", "")) == "local" else "Using cloud AI provider with explicit consent; bounded project/catalog metadata will be sent to the configured endpoint."


static func _bounded_tool_content(value: Variant, max_chars: int) -> String:
    return _bounded_text(JSON.stringify(value), max_chars)


static func _bounded_text(value: String, max_chars: int) -> String:
