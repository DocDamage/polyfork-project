...
func _physics_process(delta: float) -> void:
    ...
    if look.length_squared() > 0.0001:
        _apply_look(look.x * stick_look_speed * delta, look.y * stick_look_speed * delta)
    ...
    elif jump_enabled and Input.is_action_just_pressed(GameplayInput.JUMP):
...
func _apply_look(yaw_delta: float, pitch_delta: float) -> void:
    _yaw -= yaw_delta
    _pitch = clamp(_pitch - pitch_delta, -1.45, 1.45)
    rotation.y = _yaw
    if _camera != null:
...
func _ensure_camera() -> void:
    ...
    _camera.position = Vector3(0.0, 1.58, 0.0)
