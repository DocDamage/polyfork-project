...
static func run_checks() -> Array[String]:
    ...
    var root: String = "user://tests/project_repository_%s" % StableId.generate()
    ...
    var manifest_path: String = str(create_result["manifest_path"])
    if not FileAccess.file_exists(manifest_path):
        errors.append("Creating a project must persist project.json.")
        return errors

    var first_text: String = FileAccess.get_file_as_string(manifest_path)
    ...
    var saved_text: String = FileAccess.get_file_as_string(manifest_path)
    project.title = ""
    var invalid_save: Dictionary = repository.save_project(project)
    ...
    var invalid_open: Dictionary = repository.open_project("not-a-project-id")
    if invalid_open.get("ok", false):
        errors.append("Repository must reject malformed project IDs before filesystem access.")

    var temp_dir: String = repository.get_project_directory(create_result["project"].project_id)
...
static func _check_invalid_manifest_rejection(repository, errors: Array[String]) -> void:
    var invalid_id: String = StableId.generate()
    var invalid_dir: String = repository.get_project_directory(invalid_id)
    ...
    var open_result: Dictionary = repository.open_project(invalid_id)
