...
static func run_checks() -> Array[String]:
    ...
    var bind: Dictionary = session.bind_project(project, func() -> Dictionary:
        dirty_count[0] += 1
    ...
    session.begin_proxy_placement("Gameplay Prop")
    var placed: Dictionary = session.commit_placement()
    ...
    var root := "user://tests/phase6_components_%s" % StableId.generate()
    var service = GameplayService.new()
    var service_bind: Dictionary = service.bind_project(project, root.path_join("project"), session, func() -> Dictionary:
        dirty_count[0] += 1
        return {"ok": true, "errors": []}
    )
