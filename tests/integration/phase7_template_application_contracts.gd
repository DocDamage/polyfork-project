...
static func _materialize_template(registry, template_id: String) -> Array[String]:
    var errors: Array[String] = []
    var root := "user://tests/phase7_apply_%s_%s" % [template_id, StableId.generate()]
    ...
    var project = create_result["project"]; var cell_id := StableId.generate(); project.cell_ids = [cell_id]
    var session = EditorSession.new(); var dirty := [0]
    ...
    for starter in manifest.get("starter_entities", []):
        var key := str(starter.get("starter_key", "")); var expected := StableId.from_seed("template:%s:%s:%s" % [project.project_id, template_id, key])
        ...
        var record := _find_entity(project.entity_records, expected)
    ...
    if template_id == "driving":
        var vehicle_id := str(starter_ids.get("vehicle_prototype", "")); var vehicle_components := gameplay.components_for_entity(vehicle_id)
    ...
    var before_count := project.entity_records.size(); var second: Dictionary = application.materialize_starters(project, manifest, session, gameplay, func(_position: Vector3) -> String: return cell_id)
...
static func _module_editability(registry) -> Array[String]:
    ...
    var project = project_result["project"]; var service = ProjectModuleService.new(); var original_template := project.template_id
