...
static func run_checks() -> Array[String]:
    ...
    var project_id := StableId.generate(); var build_id := StableId.generate(); var entity_id := StableId.generate(); var cell_id := StableId.generate()
    var asset_a := StableId.generate(); var asset_b := StableId.generate(); var source_id := StableId.generate()
    var project := {"project_id": project_id, "title": "Export Test", "world_profile": "small", "template_id": "blank_sandbox", "cell_ids": [cell_id], "entities": [{"entity_id": entity_id, "asset_id": asset_a}], "dependencies": [asset_a]}
    ...
    var bad_project := project.duplicate(true); bad_project["entities"] = [{"entity_id": entity_id, "asset_id": "not-a-stable-id"}]
    ...
    var source_root := "user://tests/phase13/assets-%s" % StableId.generate()
    DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path(source_root))
    var source_file := source_root.path_join("models/tree.glb")
    DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path(source_file.get_base_dir()))
    var handle := FileAccess.open(source_file, FileAccess.WRITE)
    if handle == null: errors.append("Phase 13 fixture source asset must be writable.")
    else: handle.store_string("fixture"); handle.close()
    var hash64 := "a".repeat(64)
    var catalog := [{"asset_id": asset_a, "source_id": source_id, "relative_path": "models/tree.glb", "asset_type": "glb", "content_hash": hash64, "size_bytes": 7, "missing": false, "license": {"spdx": "CC0-1.0", "author": "Fixture", "source_url": "", "notes": ""}}]
    var sources := [{"source_id": source_id, "root_path": source_root}]
    ...
    var missing_catalog := catalog.duplicate(true); missing_catalog[0]["missing"] = true
    ...
    var unknown_license := resolution.get("dependencies", []).duplicate(true)
    ...
    var runtime_paths := {"src/runtime/play_session.gd": true, "src/editor/runtime_entity_bridge.gd": true}
    ...
    var serialized_plan := JSON.stringify(data_plan)
    ...
    var dependency_entries := resolution.get("dependencies", []) if resolution.get("ok", false) else []
    var manifest := Contracts.new_manifest(project, build_id, "ExportTest", dependency_entries, [{"package_path": "runtime_data/project.json", "classification": Contracts.RUNTIME_REQUIRED}])
    ...
    var unsafe := manifest.duplicate(true); unsafe["files"] = [{"package_path": "../escape.txt", "classification": Contracts.RUNTIME_REQUIRED}]
    if Contracts.validate_manifest(unsafe).is_empty(): errors.append("Export manifests must reject traversal package paths.")
    var secret := manifest.duplicate(true); secret["api_key"] = "do-not-ship"
