...
static func run_checks() -> Array[String]:
    ...
    var local: Dictionary = {
        ...
        "protocol": "openai_compatible_chat_v1",
        "scope": "local",
    ...
    var secret_descriptor: Dictionary = local.duplicate(true)
    secret_descriptor["api_key"] = "must-not-persist"
    ...
    var remote_local: Dictionary = local.duplicate(true)
    remote_local["endpoint"] = "http://example.com/v1/chat/completions"
    ...
    var cloud: Dictionary = local.duplicate(true)
    cloud["provider_id"] = "cloud-test"; cloud["scope"] = "cloud"; cloud["endpoint"] = "https://example.com/v1/chat/completions"; cloud["credential_env"] = "POLYFORK_TEST_AI_KEY"
    ...
    if not load_result.get("ok", false): errors.append("AI provider registry must create user-scoped settings.")
        ...
        registry.set_privacy_policy({"local_only": false})
        blocked = registry.resolve_active_provider()
    ...
    if QueryService == null or ActionRegistry == null or PreviewService == null or GraphBuilder == null or StagingService == null or ExecutionService == null or OpenAiCompatibleProvider == null:
        errors.append("Phase 12 AI foundation modules must preload successfully.")
    return errors
