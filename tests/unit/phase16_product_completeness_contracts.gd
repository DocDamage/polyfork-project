...
static func _check_shell_and_biome(tree: SceneTree) -> Array[String]:
    var errors: Array[String] = []
    var storage_root := "user://tests/phase16/shell-%s" % StableId.generate()
    ...
    await tree.process_frame
    var home := app.get_node_or_null("HomeScreen")
    if home == null: errors.append("Home screen is missing from Main.tscn.")
        ...
        for button_name in ["WorldsButton", "TemplatesButton", "AssetLibraryButton"]:
            var button := home.find_child(button_name, true, false) as Button
        ...
        if worlds != null:
            ...
            var overlay := home.get_node_or_null("PlayWorldHomeCreatorOverlay")
            if overlay == null or not overlay.visible: errors.append("My Worlds did not open a real creator surface.")
        ...
        if templates != null:
            ...
            var overlay := home.get_node_or_null("PlayWorldHomeCreatorOverlay")
            if overlay == null or not overlay.visible: errors.append("Templates did not open a real creator surface.")
            if overlay != null: overlay.call("close")
    var new_world := NewWorldScene.instantiate() as Control
    tree.root.add_child(new_world); await tree.process_frame
    var biome := new_world.find_child("BiomeOption", true, false) as OptionButton
    if biome == null or biome.item_count < 3: errors.append("New World does not expose the required biome presets.")
    var captured: Dictionary = {}
    new_world.create_requested.connect(func(value: Dictionary): captured = value.duplicate(true))
...
static func _check_semantic_search() -> Array[String]:
    ...
    var ranked := SemanticSearch.new().rank(records, "car")
...
static func _check_real_thumbnail() -> Array[String]:
    var errors: Array[String] = []
    var root := "user://tests/phase16/assets-%s" % StableId.generate()
    var source := root.path_join("source")
    var project := root.path_join("project")
    ...
    var scene_path := source.path_join("preview_box.tscn")
    ...
    var records := library.get_records(false)
    ...
    var image := Image.load_from_file(str(thumbnail.get("path", "")))
