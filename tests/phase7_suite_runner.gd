...
const ScalePerformanceContracts = preload("res://tests/integration/phase7_scale_performance_contracts.gd")
...
func _run() -> void:
    ...
    match suite:
        ...
        "play":
            errors.append_array(PlayStateContracts.run_checks(root))
            errors.append_array(ScalePerformanceContracts.run_checks(root))
